...
const DataTable = () => {
    ...
    const [filterAccordionIsOpen, setFilterAccordionIsOpen] = useState<boolean>(false);
    const [sortAccordionIsOpen, setIsSortAccordionIsOpen] = useState<boolean>(false);
    const [columnFilters, setColumnFilters] = useState<ColumnFiltersState>([]);

    const columnNames = Object.keys(data[0]);

    document.documentElement.style.setProperty("--columnCount", String(columnNames.length));

    const columns = useMemo<ColumnDef<unknown>[]>(
        () => columnNames.map((columnName) => {
            return {
                accessorFn: (row: Row<unknown>) => row[columnName as keyof unknown],
                id: columnName,
    ...
    const defaultColumn = {
        cell: function Cell ({ getValue, row: { index }, column: { id }, table }: CellPropsType) {
